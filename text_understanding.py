...
def find_answer(text, question):
    ...
    return max(sentences, key=lambda s: sum(k in s.lower() for k in keywords), default="Sorry, I couldn't find an answer.")
...
def generate_quiz_questions(text, num_questions=5):
    ...
    for sentence in sentences:
        ...
        if words:
            ...
            question = sentence.replace(chosen, "_", 1)
            questions.append((question, chosen))
...
if main_text:
    ...
    if st.button("Get Answer") and question:
        ...
        answer_trans = translator.translate(answer_en, dest=selected_lang_code).text if selected_lang_code != "en" else answer_en

        st.subheader("✅ Answer:")
        st.success(answer_trans)

        if voice_enabled:
            st.subheader("🔊 Listen to Answer")
            audio_file = speak_text(answer_trans, selected_lang_code)
            st.audio(audio_file)
            with open(audio_file, "rb") as af:
                st.download_button("📅 Download Answer Audio", af, file_name="answer.mp3")

        st.download_button("📄 Download Answer as Text", answer_trans, file_name="answer.txt")

    if st.button("Summarize Text"):
        detected_lang = detect(main_text)
        main_text_en = translator.translate(main_text, dest="en").text if detected_lang != 'en' else main_text
        summary_en = summarize_text(main_text_en)
        summary_trans = translator.translate(summary_en, dest=selected_lang_code).text if selected_lang_code != "en" else summary_en

        st.subheader("📝 Summary")
        st.info(summary_trans)

        if voice_enabled:
            st.subheader("🔊 Listen to Summary")
            audio_file = speak_text(summary_trans, selected_lang_code)
            st.audio(audio_file)
            with open(audio_file, "rb") as af:
                st.download_button("📅 Download Summary Audio", af, file_name="summary.mp3")

        st.download_button("📄 Download Summary as Text", summary_trans, file_name="summary.txt")
    ...
    if st.button("Generate Quiz Questions"):
        ...
        for i, (q, a) in enumerate(questions, 1):
            trans_q = translator.translate(q, dest=selected_lang_code).text if selected_lang_code != 'en' else q
            trans_a = translator.translate(a, dest=selected_lang_code).text if selected_lang_code != 'en' else a
            st.markdown(f"Q{i}: {trans_q}")
            with st.expander("Show Answer"):
                st.write(f"✅ {trans_a}")
            if voice_enabled:
                audio_file = speak_text(trans_q, selected_lang_code)
                st.audio(audio_file)
